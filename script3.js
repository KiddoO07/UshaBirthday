function init() {
  ...
  let bannerContent = 'Oiiiii , ushaaa kese ho hehe heres\n a surprise for yea hehe :3'
  const banners = []
  let spriteId = 0
  let topIndex = 3
  let count = 0
  ...
  const createPlanesWithText = (texts) => {
  let localIndex = 0;

  const step = () => {
    const text = texts[localIndex];
    const bannerText = text.endsWith('#') ? text : text + '#';

    if (bannerText !== '#') createPlane(bannerText.replaceAll('#', ' '));

    localIndex = (localIndex + 1) < texts.length ? localIndex + 1 : 0;

    if (localIndex === 0) spriteId = 0; // reset for new loop

    setTimeout(step, 4000); // Delay between planes
  };
  ...
  createPlanesWithText([
  "So apart from small gift I gave",
  "I wanted to give you one more thing",
  "A pet hehe (it will follow you whereever you click on the screen)",
  "But you got to earn that by solving a puzzle :) "
]);
